Public Class frmMasterPelanggan
    ...
    Sub Simpan()
        ...
        If ID = 0 Then
            'input data baru
            EksekusiSQL("INSERT INTO `pelanggan`(`NAMA_PELANGGAN`, `ALAMAT`, `NOMOR_TELEPON`) VALUES ('" & txtNama.Text & "','" & txtAlamat.Text & "','" & txtNomorTelepon.Text & "')")
            Segarkan()
            MessageBox.Show("Data sukses ditambahkan.", "Informasi", MessageBoxButtons.OK, MessageBoxIcon.Information)
        Else
            'ubah data lama
            EksekusiSQL("UPDATE `pelanggan` SET `NAMA_PELANGGAN`='" & txtNama.Text & "',`ALAMAT`='" & txtAlamat.Text & "',`NOMOR_TELEPON`=" & txtNomorTelepon.Text & "' WHERE `ID_PELANGGAN`='" & ID & "'")
            Segarkan()
